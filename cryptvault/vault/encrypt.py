...
def encrypt_secrets(in_secrets: list[Secret], secret_key: bytes) -> list[EncryptedSecret]:
    encrypted_secrets = []
    for secret in in_secrets:
        fresh_bytes  = secrets.token_bytes(nbytes = 12)
        cipher       = fresh_bytes + AESGCM(key = secret_key).encrypt(fresh_bytes, bytes(f"{secret.value}", encoding = 'utf-8'), b"")
        encrypted_secrets.append(EncryptedSecret(key = secret.key, value = cipher))

    return encrypted_secrets


def decrypt_secret(secret: EncryptedSecret, secret_key: bytes) -> Secret:   
    decrypted_secret = AESGCM(key = secret_key).decrypt(secret.value[:12], secret.value[12:], b"")
